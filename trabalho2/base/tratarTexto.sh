...
#
# Dado um texto de entrada (pela stdin ou caminho do arquivo que contém o texto),
# esse script separa cada palavra do texto em linhas,
# i.e., uma palavra por linha, sem pontuação (e linhas sem número);
# Converte todas as letras para minúsculas (exceto o id de página "PA").
# E remove as stop words brasileiras
# Exibe na stdout.
#



inputStream="${1:-/dev/stdin}"
[ -r "$inputStream" ]  && {

	egrep -o '\w+(-\w+)?' $inputStream |
	sed '/[0-9]/d' |
	sed '
		/\<PA\>/! {
